...
try {
    ...
    if ($inscripcion_existente && $inscripcion_existente['estado'] === 'Inscrito') {
        ...
        echo json_encode(['error' => 'Ya estás inscrito en este evento.']);
        exit;
    }
    ...
    if ($evento['cupo_actual'] >= $evento['cupo_maximo']) {
        $pdo->rollBack();
        http_response_code(409); 
        echo json_encode(['error' => 'El cupo para este evento está lleno.']);
        exit;
    }

    
    if ($inscripcion_existente) { 
        $stmt = $pdo->prepare("UPDATE inscripciones SET estado = 'Inscrito', fecha_inscripcion = NOW() WHERE id_inscripcion = :id_inscripcion");
        $stmt->bindParam(':id_inscripcion', $inscripcion_existente['id_inscripcion'], PDO::PARAM_INT);
        $stmt->execute();
    } else {
        $stmt = $pdo->prepare("INSERT INTO inscripciones (id_usuario, id_evento, fecha_inscripcion, estado) VALUES (:id_usuario, :id_evento, NOW(), 'Inscrito')");
    ...
    echo json_encode(['error' => 'Error al procesar la inscripción: ' . $e->getMessage()]);
}
?>